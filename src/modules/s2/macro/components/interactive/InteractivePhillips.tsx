...
export function InteractivePhillips() {
  ...
  return (
    <div className="bg-white dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-white/10 p-6 my-8">
      <div className="flex items-center justify-between mb-2">
        <h3 className="text-xl font-semibold">Courbe de Phillips</h3>
        <div className="flex items-center gap-2">
          {/* Toggle Mode - Désactivé pour production
          <div className="flex bg-slate-100/80 dark:bg-white/5 rounded-lg p-1">
            <button onClick={() => setMode('theorique')}
              className={`px-3 py-1.5 rounded-md text-sm font-medium flex items-center gap-1.5 transition-colors ${mode === 'theorique' ? 'bg-white dark:bg-slate-900/80 shadow text-red-600' : 'text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:text-white'}`}>
              <BookOpen size={14} /> Théorique
            </button>
            <button onClick={() => setMode('numerique')}
              className={`px-3 py-1.5 rounded-md text-sm font-medium flex items-center gap-1.5 transition-colors ${mode === 'numerique' ? 'bg-white dark:bg-slate-900/80 shadow text-emerald-600' : 'text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:text-white'}`}>
              <Calculator size={14} /> Numérique
            </button>
          </div>
          */}
          <button onClick={() => setShowLegend(!showLegend)}
            className={`px-3 py-1.5 rounded-lg text-sm flex items-center gap-1.5 ${showLegend ? 'bg-red-100 text-red-600' : 'bg-slate-100/80 dark:bg-white/5 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:bg-white/10'}`}>
      ...
      {showLegend && (
        <div className="mb-4 p-3 bg-red-50 dark:bg-red-950/30 rounded-lg text-sm border border-red-100">
      ...
      <div className="mb-4">
        <button onClick={() => setShowIntuition(!showIntuition)}
          className={`px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-colors ${showIntuition ? 'bg-gray-900 text-white' : 'bg-slate-100/80 dark:bg-white/5 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:bg-white/10'}`}>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        <button onClick={() => setEvent('none')}
          className={`px-3 py-1.5 rounded-lg text-sm font-medium flex items-center gap-1 ${event === 'none' ? 'bg-gray-900 text-white' : 'bg-slate-100/80 dark:bg-white/5 hover:bg-slate-200 dark:bg-white/10'}`}>
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        <div className="lg:col-span-3">
          <svg viewBox={`0 0 ${W} ${H}`} className="w-full bg-slate-100/50 dark:bg-white/5 rounded-xl border border-gray-100">
            {/* Grille */}
          ...
          {mode === 'numerique' && event !== 'none' && (
            <div className="mt-4 p-3 bg-emerald-50 dark:bg-emerald-950/30 rounded-lg border border-emerald-200">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <p className="text-slate-700 dark:text-slate-300">Avant (E₀) :</p>
                  <p className="font-mono text-emerald-700">u₀ = {currentUn}%, π₀ = {pi0.toFixed(1)}%</p>
                </div>
                <div>
                  <p className="text-slate-700 dark:text-slate-300">Après (E₁) :</p>
        ...
        <div className="lg:col-span-2 space-y-4">
          {expl ? (
              ...
              <div className="space-y-3">
                {expl.steps.map((s, i) => (
                  <div key={i} className="flex items-start gap-2">
                    <span className="w-5 h-5 rounded-full bg-red-200 dark:bg-red-950/30 text-red-800 text-xs flex items-center justify-center shrink-0 mt-0.5">{i + 1}</span>
                    <div>
            ...
            <div className="bg-slate-100/50 dark:bg-white/5 rounded-xl p-4 border border-slate-200 dark:border-white/10">
              <p className="font-medium text-slate-800 dark:text-slate-200 mb-2">Équilibre (u = uₙ, π = πᵉ)</p>
              <p className="text-sm text-slate-700 dark:text-slate-300 mb-3">Clique sur un événement pour voir l'arbitrage inflation-chômage.</p>
              <div className="space-y-2 text-sm">
                <div className="flex items-center gap-2"><div className="w-6 h-1 bg-blue-500 dark:bg-blue-950/30 rounded" /><span><strong>CT</strong> : Court terme (décroissante)</span></div>
                <div className="flex items-center gap-2"><div className="w-6 h-1 bg-red-500 dark:bg-red-950/30 rounded" style={{ backgroundImage: 'repeating-linear-gradient(90deg, #dc2626 0, #dc2626 6px, transparent 6px, transparent 10px)' }} /><span><strong>LT</strong> : Long terme (verticale en uₙ)</span></div>
              </div>
              {mode === 'numerique' && (
                <div className="mt-3 pt-3 border-t border-slate-200 dark:border-white/10 text-sm text-slate-700 dark:text-slate-300">
          ...
          <div className="space-y-3">
            <div className="p-4 bg-slate-100/50 dark:bg-white/5 rounded-lg border border-slate-200 dark:border-white/10 relative">
            ...
            <div className="p-4 bg-slate-100/50 dark:bg-white/5 rounded-lg border border-slate-200 dark:border-white/10 relative">
              ...
              <div className="space-y-1 text-sm text-slate-800 dark:text-slate-200">
                ...
                <p className="text-slate-600 dark:text-slate-400">Si u &lt; uₙ maintenu → π accélère sans fin</p>
              </div>
            </div>
          </div>
        </div>
