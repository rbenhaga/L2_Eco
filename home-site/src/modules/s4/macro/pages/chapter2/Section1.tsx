import { Section, Callout, Math, FormulaBox, FigurePlaceholder } from '../../../../../components';
        ...
        <section id="ch2-section-1" data-section-title="1. Marché des biens en économie ouverte" className="mb-16">
            ...
            <div id="ch2-s1-4" className="mb-12">
                ...
                <Section type="exemple" title="Exemple numérique">
                    <p className="mb-2"><strong>Données initiales (t₀) :</strong> <Math>{'E_{\\text{USD}/\\text{EUR}} = 1'}</Math>, <Math>{'P = 10\\,\\text{EUR}'}</Math>, <Math>{'P^* = 10\\,\\text{USD}'}</Math>, donc <Math>{'\\varepsilon = 1'}</Math>. X = 100 et M = 100.</p>
                    <p className="mb-2"><strong>État initial :</strong> <Math>{'NX_0 = X - M/\\varepsilon = 100 - 100/1 = 0\\,\\text{EUR}'}</Math> (équilibre commercial)</p>
                    <p className="mb-2"><strong>Court terme (t₁), ε' = 0,8 :</strong> Les volumes ne bougent pas.</p>
                    <FormulaBox>{`NX_{CT} = 100 - \\frac{100}{0{,}8} = 100 - 125 = -25\\,\\text{EUR}`}</FormulaBox>
                    <p className="mb-2"><strong>Moyen terme (t₂) :</strong> Ajustement des comportements : X = 140, M = 80.</p>
                    <FormulaBox>{`NX_{MT} = 140 - \\frac{80}{0{,}8} = 140 - 100 = +40\\,\\text{EUR}`}</FormulaBox>
                </Section>

                                {/* Graph placeholder: depreciation example */}
                <FigurePlaceholder
                    title="Figure 2 - Evolution du solde NX dans l'exemple chiffre"
