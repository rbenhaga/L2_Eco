...
export function ModuleHub({
    ...
    const getHoverTextClass = () => {
        switch (themeColor) {
            case 'emerald': return 'group-hover:text-emerald-600 dark:group-hover:text-emerald-400';
            case 'violet': return 'group-hover:text-violet-600 dark:group-hover:text-violet-400';
            case 'green': return 'group-hover:text-green-600 dark:group-hover:text-green-400';
            case 'amber': return 'group-hover:text-amber-600 dark:group-hover:text-amber-400';
    ...
    const getButtonClass = (variant: string) => {
        switch (variant) {
            ...
            case 'secondary':
                return 'bg-white text-slate-900 border border-slate-200 hover:bg-slate-50 dark:bg-slate-800 dark:text-white dark:border-slate-700 dark:hover:bg-slate-700';
            case 'outline':
                return 'bg-transparent text-slate-700 dark:text-white border border-slate-300 dark:border-white/20 hover:bg-slate-100 dark:hover:bg-white/10';
            default:
    ...
    return (
        <div className="animate-in fade-in duration-700">
            <header className="pt-8 sm:pt-16 pb-12 sm:pb-20 text-center px-4 sm:px-6 relative">
                ...
                <p className={`text-xs sm:text-sm font-bold uppercase tracking-[0.2em] mb-4 text-${themeColor === 'default' ? 'indigo' : themeColor}-500 dark:text-${themeColor === 'default' ? 'indigo' : themeColor}-400`}>
                    Partiel 2024-2025
                </p>

                <h1 className="text-3xl sm:text-5xl md:text-6xl font-bold tracking-tight text-slate-900 dark:text-white mb-6">
                    {title}
                </h1>

                <p className="text-base sm:text-lg md:text-xl text-slate-600 dark:text-slate-400 max-w-2xl mx-auto leading-relaxed mb-8 sm:mb-12 font-medium">
                    {description}
                </p>

                <div className="inline-flex flex-wrap justify-center items-center gap-3 text-xs sm:text-sm text-slate-600 dark:text-slate-400 mb-8 sm:mb-12 bg-white/60 dark:bg-slate-900/40 backdrop-blur-md px-6 py-3 rounded-full border border-slate-200 dark:border-white/10 shadow-sm">
                    {stats.map((stat, index) => (
                        <div key={index} className="flex items-center gap-3">
                            <span className="text-slate-900 dark:text-white font-semibold"><span className={`text-${themeColor === 'default' ? 'blue' : themeColor}-500 dark:text-${themeColor === 'default' ? 'blue' : themeColor}-400`}>{stat.value}</span> {stat.label}</span>
                            {index < stats.length - 1 && <span className="w-1 h-1 rounded-full bg-slate-300 dark:bg-slate-600" />}
                        </div>
                    ))}
                </div>
                    ...
                    {(section.title || section.badge) && (
                        <div className={section.badge ? "flex items-center gap-4 mb-8" : "text-center mb-8"}>
                            ...
                            <div className={section.badge ? "" : "text-center w-full"}>
                                <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white mb-1">{section.title}</h2>
                    ...
                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
                        {section.items.map((ch) => (
                            <Link
                                ...
                                to={ch.path}
                                className={`
                                    group relative p-6 sm:p-8 rounded-3xl border border-slate-200 dark:border-white/5 bg-white/60 dark:bg-slate-900/40 backdrop-blur-md 
                                    hover:bg-white/80 dark:hover:bg-slate-800/60 ${getHoverBorderClass()} transition-all duration-300 no-underline 
                            ...
                            >
                                ...
                                <h3 className={`text-xl font-bold text-slate-900 dark:text-white mb-3 transition-colors ${getHoverTextClass()}`}>
                                    {ch.title}
                                </h3>
                                <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed border-t border-slate-200 dark:border-white/5 pt-3 mt-auto">
                                    {ch.desc}
                                </p>
                            </Link>
                        ))}
            ...
            <section className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-16">
                <div className="relative p-1 bg-linear-to-br from-slate-200 dark:from-white/10 to-transparent rounded-[2.5rem]">
                    <div className="bg-white/50 dark:bg-slate-950/50 backdrop-blur-xl rounded-[2.3rem] p-8 sm:p-12 border border-slate-200 dark:border-white/5">
                        <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white mb-8 text-center">{extras.title}</h2>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 justify-center">
                            {extras.items.map((item) => (
                                <Link
                                    key={item.path}
                                    to={item.path}
                                    className="p-4 sm:p-5 rounded-2xl bg-white/40 dark:bg-white/5 border border-slate-200 dark:border-white/5 hover:border-slate-300 dark:hover:border-white/20 hover:bg-white/60 dark:hover:bg-white/10 transition-all no-underline group flex flex-col items-center text-center gap-3 active:scale-[0.98]"
                                >
