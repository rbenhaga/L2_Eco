...
export default function AdminDashboard() {
    ...
    const formatExpiryLabel = (hours?: number) => {
        const value = Number(hours);
        if (!Number.isFinite(value)) return '7 jours';
        if (value < 0) return 'Sans expiration';
        if (value === 24) return '24 heures';
        if (value === 72) return '3 jours';
        if (value === 168) return '7 jours';
        if (value === 720) return '30 jours';
        return `${value} heures`;
    };
    void formatExpiryLabel;
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        <div className="min-w-0">
                            ...
                            <p className="text-xs sm:text-sm hidden sm:block" style={{ color: 'var(--color-text-muted)' }}>
                                Pilotage clair, mise à jour automatique toutes les 15 secondes.
                            </p>
                        </div>
                        <div className="flex w-full sm:w-auto items-center justify-end gap-2">
                            <div className="hidden lg:flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs" style={{ background: 'var(--color-bg-overlay)', border: '1px solid var(--color-border-default)', color: 'var(--color-text-secondary)' }}>
                                <span>Sync auto 15s</span>
                                <span style={{ color: 'var(--color-text-muted)' }}>•</span>
                                <span>{lastSyncedAt ? `Dernière sync: ${new Date(lastSyncedAt).toLocaleTimeString('fr-FR')}` : 'En attente de sync'}</span>
                            </div>
                            <button
                        ...
                        <div className="rounded-2xl p-6" style={{ background: 'var(--color-bg-raised)', border: '1px solid var(--color-border-default)' }}>
                            <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                                <div>
                                    ...
                                    <p className="text-sm mt-1" style={{ color: 'var(--color-text-muted)' }}>
                                        Gère les notifications visibles dans la cloche.
                                    </p>
                                </div>
                                <button
                            ...
                            <div className="space-y-3">
                                {filteredNotifications.map((notif) => {
                                    ...
                                    const createdAt = Number(notif.createdAt) > 0 ? Number(notif.createdAt) : Date.now();
                                    const hours = Number(notif.expiresInHours);
                                    const hasFiniteExpiry = Number.isFinite(hours) && hours >= 0;
                                    const expiresAt = hasFiniteExpiry ? createdAt + hours * 60 * 60 * 1000 : null;
                                    ...
                                    >
                                        <div className="mb-2 flex flex-wrap items-center justify-between gap-2 text-xs">
                                            <span style={{ color: 'var(--color-text-secondary)' }}>
                                                Durée: {formatExpiryLabel(hours)}
                                            </span>
                                            <span style={{ color: isExpired ? 'var(--color-warning)' : 'var(--color-info)' }}>
                                                {expiresAt ? `Expire le ${new Date(expiresAt).toLocaleString('fr-FR')}` : 'Sans expiration'}
                                            </span>
                                        </div>
                                        <div className="grid grid-cols-1 md:grid-cols-7 gap-2">
                                            ...
                                            />
                                            <select
                                                value={Number.isFinite(hours) ? String(hours) : '168'}
                                                onChange={(e) => updateNotification(index, 'expiresInHours', Number(e.target.value))}
                                                className="h-10 px-3 rounded-lg text-sm"
                                                style={{ border: '1px solid var(--color-border-default)', background: 'var(--color-bg-raised)', color: 'var(--color-text-primary)' }}
                                            >
                                                ...
                                                <option value="-1">Sans expiration</option>
                                            </select>
                                            <button
