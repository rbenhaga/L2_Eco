...
export function InteractiveWSPS() {
  ...
  return (
    <div className="bg-white dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-white/10 p-6 my-8">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex bg-slate-100/80 dark:bg-white/5 rounded-lg p-1">
            <button onClick={() => setMode('theorique')}
              className={`px-3 py-1.5 rounded-md text-sm font-medium flex items-center gap-1.5 transition-colors ${mode === 'theorique' ? 'bg-white dark:bg-slate-900/80 shadow text-purple-600' : 'text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:text-white'}`}>
          ...
          */}
          <button onClick={() => setShowLegend(!showLegend)}
            className={`px-3 py-1.5 rounded-lg text-sm flex items-center gap-1.5 ${showLegend ? 'bg-purple-100 text-purple-600' : 'bg-slate-100/80 dark:bg-white/5 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:bg-white/10'}`}>
      ...
      {showLegend && (
        <div className="mb-4 p-3 bg-purple-50 dark:bg-purple-950/30 rounded-lg text-sm border border-purple-100">
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          {expl ? (
            <div className="bg-purple-50 dark:bg-purple-950/30 rounded-xl p-4 border border-purple-100">
              <p className="font-semibold text-purple-900 mb-3">{expl.title}</p>
              <div className="space-y-3">
                {expl.steps.map((s, i) => (
                  <div key={i} className="flex items-start gap-2">
                    <span className="w-5 h-5 rounded-full bg-purple-200 dark:bg-purple-950/30 text-purple-800 text-xs flex items-center justify-center shrink-0 mt-0.5">{i + 1}</span>
                    <div>
            ...
            <div className="bg-slate-100/50 dark:bg-white/5 rounded-xl p-4 border border-slate-200 dark:border-white/10">
              <p className="font-medium text-slate-800 dark:text-slate-200 mb-2">État initial</p>
              <p className="text-sm text-slate-700 dark:text-slate-300 mb-3">Clique sur un événement pour voir les effets sur uₙ.</p>
              <div className="space-y-2 text-sm">
                <div className="flex items-center gap-2"><div className="w-6 h-1 bg-purple-500 dark:bg-purple-950/30 rounded" /><span><strong>WS</strong> : Négociation salariale</span></div>
                <div className="flex items-center gap-2"><div className="w-6 h-1 bg-amber-500 dark:bg-amber-950/30 rounded" /><span><strong>PS</strong> : Fixation des prix</span></div>
              </div>
              {mode === 'numerique' && (
