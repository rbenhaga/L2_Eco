    ...
    },
    {
      ...
      solution: (
        <div className="space-y-3">
          ...
          <p className="text-slate-700 dark:text-slate-300 italic">En moyenne, on gagne 1€ par partie.</p>
    ...
    },
    {
      ...
      solution: (
        <div className="space-y-3">
          ...
          <div className="bg-blue-50 dark:bg-blue-950/30 border-l-4 border-blue-400 p-3 mb-3">
          ...
          <table className="border-collapse border border-slate-300 dark:border-white/15 text-sm">
            ...
            <tbody>
              <tr><td className="border border-slate-300 dark:border-white/15 p-2">Effectif</td><td className="border border-slate-300 dark:border-white/15 p-2">6</td><td className="border border-slate-300 dark:border-white/15 p-2">10</td><td className="border border-slate-300 dark:border-white/15 p-2">8</td><td className="border border-slate-300 dark:border-white/15 p-2">6</td><td className="border border-slate-300 dark:border-white/15 p-2">4</td><td className="border border-slate-300 dark:border-white/15 p-2">2</td></tr>
              <tr><td className="border border-slate-300 dark:border-white/15 p-2">P(X=x)</td><td className="border border-slate-300 dark:border-white/15 p-2">1/6</td><td className="border border-slate-300 dark:border-white/15 p-2">5/18</td><td className="border border-slate-300 dark:border-white/15 p-2">2/9</td><td className="border border-slate-300 dark:border-white/15 p-2">1/6</td><td className="border border-slate-300 dark:border-white/15 p-2">1/9</td><td className="border border-slate-300 dark:border-white/15 p-2">1/18</td></tr>
            </tbody>
          </table>
    ...
    },
    {
      ...
      solution: (
        <div className="space-y-4">
          ...
          <p className="text-sm text-slate-700 dark:text-slate-300">L'espérance est toujours linéaire (indépendance ou non) :</p>
          <M>{"E(W) = E(2X - Y) = 2E(X) - E(Y) = 2(2) - 3 = 1"}</M>
          <M>{"E(W + 4Z) = E(W) + 4E(Z) = 1 + 4(-2) = \\boxed{-7}"}</M>
          
          <p><strong>2. V(W + 4Z):</strong></p>
          <p className="text-sm text-slate-700 dark:text-slate-300">Formule générale : V(A + B) = V(A) + V(B) + 2Cov(A,B)</p>
          
          <p className="text-sm mt-2"><em>Étape 1 : Calculer V(W)</em></p>
          <p className="text-sm text-slate-700 dark:text-slate-300">V(aX + bY) = a²V(X) + b²V(Y) + 2ab·Cov(X,Y)</p>
          ...
          <p className="text-sm text-slate-700 dark:text-slate-300">Bilinéarité : Cov(aU, bV) = ab·Cov(U,V)</p>
          <M>{"Cov(W, 2Y) = 2 \\cdot Cov(W, Y) = 2 \\cdot Cov(2X - Y, Y)"}</M>
          <p className="text-sm text-slate-700 dark:text-slate-300">On développe Cov(2X - Y, Y) :</p>
          <M>{"Cov(2X - Y, Y) = 2Cov(X,Y) - Cov(Y,Y) = 2(0) - V(Y) = -5"}</M>
          <M>{"Cov(W, 2Y) = 2(-5) = \\boxed{-10}"}</M>
          
          <p><strong>4. r(W, Y):</strong></p>
          <p className="text-sm text-slate-700 dark:text-slate-300">Coefficient de corrélation : r(A,B) = Cov(A,B) / (σ_A · σ_B)</p>
          ...
          <p className="text-sm text-slate-700 dark:text-slate-300 mt-1">r &lt; 0 : W et Y sont négativement corrélés (logique car W = 2X - Y contient -Y)</p>
        </div>
      ),
    },
  ],
