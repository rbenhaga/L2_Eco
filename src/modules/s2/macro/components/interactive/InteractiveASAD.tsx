    ...
    <div className="bg-white dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-white/10 p-6 my-8">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex bg-slate-100/80 dark:bg-white/5 rounded-lg p-1">
            <button onClick={() => setMode('theorique')}
              className={`px-3 py-1.5 rounded-md text-sm font-medium flex items-center gap-1.5 transition-colors ${mode === 'theorique' ? 'bg-white dark:bg-slate-900/80 shadow text-orange-600' : 'text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:text-white'}`}>
          ...
          */}
          <button onClick={() => setShowLegend(!showLegend)}
            className={`px-3 py-1.5 rounded-lg text-sm flex items-center gap-1.5 ${showLegend ? 'bg-orange-100 text-orange-600' : 'bg-slate-100/80 dark:bg-white/5 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:bg-white/10'}`}>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <button onClick={() => { setScenario('none'); setPhase(0); }}
          className={`px-3 py-1.5 rounded-lg text-sm font-medium flex items-center gap-1 ${scenario === 'none' ? 'bg-gray-900 text-white' : 'bg-slate-100/80 dark:bg-white/5 hover:bg-slate-200 dark:bg-white/10'}`}>
          ...
          <div className="flex gap-1">
            {[0, 1, 2].slice(0, maxPhase + 1).map(p => (
              <button key={p} onClick={() => { setPhase(p as 0|1|2); setIsPlaying(false); }}
                className={`px-3 py-1.5 rounded text-sm font-medium ${phase === p ? 'bg-orange-500 text-white' : 'bg-slate-100/80 dark:bg-white/5 hover:bg-slate-200 dark:bg-white/10'}`}>
                {p === 0 ? 't₀' : p === 1 ? 'CT' : 'MT'}
              </button>
            ))}
          </div>
          ...
          {mode === 'numerique' && scenario !== 'none' && phase > 0 && (
            <div className="mt-4 p-3 bg-emerald-50 dark:bg-emerald-950/30 rounded-lg border border-emerald-200">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  <p className="text-slate-700 dark:text-slate-300">Après ({phase === 1 ? 'E₁' : 'E₂'}) :</p>
        ...
        <div className="lg:col-span-2 space-y-4">
          {expl ? (
            ...
            <div className="bg-slate-100/50 dark:bg-white/5 rounded-xl p-4 border border-slate-200 dark:border-white/10">
              ...
              <p className="text-sm text-slate-700 dark:text-slate-300 mb-3">Clique sur un événement pour voir la dynamique CT → MT.</p>
              <div className="space-y-2 text-sm">
                <div className="flex items-center gap-2"><div className="w-6 h-1 bg-blue-500 dark:bg-blue-950/30 rounded" /><span><strong>AD</strong> : Demande agrégée</span></div>
                ...
                <div className="flex items-center gap-2"><div className="w-6 h-1 bg-red-500 dark:bg-red-950/30 rounded" style={{ backgroundImage: 'repeating-linear-gradient(90deg, #dc2626 0, #dc2626 6px, transparent 6px, transparent 10px)' }} /><span><strong>Yₙ</strong> : Production naturelle</span></div>
              </div>
              {mode === 'numerique' && (
