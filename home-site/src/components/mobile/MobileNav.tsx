...
import { DEFAULT_ENABLED_SEMESTER } from '../../config/semesterAccess';
...
export function MobileNav({ onSearchClick }: MobileNavProps) {
  ...
  const currentSemester = semesters[DEFAULT_ENABLED_SEMESTER];
        ...
        <div className="p-4 space-y-3">
          <div>
            <h3 className="text-xs font-semibold tracking-wider mb-2" style={{ color: 'var(--color-text-muted)' }}>
              {currentSemester.name.toUpperCase()}
            </h3>
            <div className="space-y-2">
              {currentSemester.subjects.map((module) => (
                <button
                  key={module.basePath}
                  onClick={() => {
