    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
            ...
            {onStartAll && (
                <button
                    onClick={onStartAll}
                    className="w-full p-6 sm:p-8 rounded-3xl border border-dashed border-slate-300 dark:border-white/20 hover:border-slate-400 dark:hover:border-white/40 bg-white/40 dark:bg-white/5 hover:bg-white/60 dark:hover:bg-white/10 transition-all group relative overflow-hidden"
                >
                    <div className="flex items-center justify-between relative z-10">
                        <div className="flex items-center gap-6">
                            <div className="w-14 h-14 rounded-2xl bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-white flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
              ...
              `}
                        >
                            <div className="flex items-start justify-between mb-4">
                                ...
                                <div className={`text-xs font-bold px-2 py-1 rounded-lg ${isSelected ? 'bg-blue-100 dark:bg-blue-500/20 text-blue-600 dark:text-blue-300' : 'bg-slate-100 dark:bg-white/5 text-slate-500'}`}>
                                    {chapter.questions.length} Q
                                </div>
                            </div>

                            <h3 className={`text-lg font-bold mb-2 ${isSelected ? 'text-blue-600 dark:text-blue-400' : 'text-slate-900 dark:text-white'}`}>
                                {chapter.title}
                            </h3>
                            <p className="text-sm text-slate-500 dark:text-slate-400 font-medium leading-relaxed">{chapter.subtitle}</p>
                        </button>
                    );
                })}
            </div>
