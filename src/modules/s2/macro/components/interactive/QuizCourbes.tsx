...
export function QuizCourbes() {
  ...
  const modelColors: Record<string, string> = {
    'IS-LM': 'bg-blue-100 dark:bg-blue-950/30 text-blue-700 border-blue-200',
    'WS-PS': 'bg-purple-100 dark:bg-purple-950/30 text-purple-700 border-purple-200',
    'AS-AD': 'bg-orange-100 text-orange-700 border-orange-200',
    'Phillips': 'bg-red-100 dark:bg-red-950/30 text-red-700 border-red-200',
  };

  const typeColors: Record<string, string> = {
    'deplacement': 'bg-cyan-100 text-cyan-700',
    'raisonnement': 'bg-amber-100 dark:bg-amber-950/30 text-amber-700',
    'calcul': 'bg-emerald-100 dark:bg-emerald-950/30 text-emerald-700',
  };
    ...
    <div className="bg-white dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-white/10 p-6 my-8">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400">Déplacements, raisonnement et calculs</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1 bg-amber-50 dark:bg-amber-950/30 px-3 py-1.5 rounded-lg border border-amber-200">
            <Trophy size={16} className="text-amber-500" />
            <span className="font-semibold text-amber-700">{score}/{filteredQuestions.length}</span>
          </div>
          <button onClick={reset} className="p-2 rounded-lg bg-slate-100/80 dark:bg-white/5 hover:bg-slate-200 dark:bg-white/10">
            <RotateCcw size={18} />
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        <span className="text-sm text-slate-600 dark:text-slate-400 self-center">Modèle :</span>
        {['all', 'IS-LM', 'WS-PS', 'AS-AD', 'Phillips'].map(f => (
          <button key={f} onClick={() => { setFilter(f); setCurrentQ(0); setSelected(null); setShowResult(false); }}
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${filter === f ? 'bg-gray-900 text-white' : 'bg-slate-100/80 dark:bg-white/5 hover:bg-slate-200 dark:bg-white/10'}`}>
            {f === 'all' ? 'Tous' : f}
          </button>
        ))}
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        ...
        {['all', 'deplacement', 'raisonnement', 'calcul'].map(t => (
          <button key={t} onClick={() => { setTypeFilter(t); setCurrentQ(0); setSelected(null); setShowResult(false); }}
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${typeFilter === t ? 'bg-gray-900 text-white' : 'bg-slate-100/80 dark:bg-white/5 hover:bg-slate-200 dark:bg-white/10'}`}>
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-sm text-slate-600 dark:text-slate-400">Question {currentQ + 1}/{filteredQuestions.length}</span>
        </div>
        <p className="text-lg font-medium text-slate-900 dark:text-white mb-3">{q.question}</p>
        ...
        {showHint && !showResult && (
          <div className="p-3 bg-amber-50 dark:bg-amber-950/30 rounded-xl border border-amber-200 mb-3">
      ...
      <div className="space-y-2 mb-6">
        {shuffledOptions.map((opt, idx) => {
          let style = 'bg-slate-100/50 dark:bg-white/5 hover:bg-slate-100/80 dark:bg-white/5 border-slate-200 dark:border-white/10';
          if (showResult) {
            if (idx === shuffledCorrectIndex) style = 'bg-green-50 border-green-300 text-green-800';
            else if (idx === selected) style = 'bg-red-50 dark:bg-red-950/30 border-red-300 text-red-800';
            else style = 'bg-slate-100/50 dark:bg-white/5 border-slate-200 dark:border-white/10 opacity-50';
          }
          return (
      ...
      {/* Explication */}
      {showResult && (
        <div className={`p-4 rounded-xl mb-4 ${selected === shuffledCorrectIndex ? 'bg-green-50 border border-green-200' : 'bg-red-50 dark:bg-red-950/30 border border-red-200'}`}>
