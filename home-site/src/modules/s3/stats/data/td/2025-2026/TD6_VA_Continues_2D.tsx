  ...
  exercises: [
    {
      ...
      solution: (
        <div className="space-y-4">
          ...
          <div className="p-4 bg-[var(--color-warning-subtle)] rounded-lg border border-[var(--color-warning)]">
            <p className="font-semibold text-[var(--color-warning)] mb-3">📐 Méthode : Détermination des 4 bornes</p>
            <div className="text-sm text-[var(--color-text-primary)] space-y-2">
              ...
              <p className="mt-2 text-[var(--color-error)] font-medium">⚠️ D_{'{Y/X}'} change selon x → marginale par morceaux !</p>
            </div>
          </div>
          ...
          <p className="text-sm text-[var(--color-text-secondary)] mb-2">On intègre sur D_{'{Y/X}'} qui dépend de x :</p>
          
          <div className="pl-4 border-l-2 border-[var(--color-info)] space-y-2">
          ...
          <div className="p-3 bg-[var(--color-success-subtle)] rounded border border-[var(--color-success)] mt-2">
            <M>{"a(x) = \\begin{cases} \\frac{5x^4}{132} & 0 \\leq x \\leq 2 \\\\ \\frac{10x}{33} & 2 < x \\leq 3 \\end{cases}"}</M>
          </div>

          <p><strong>Marginale b(y) :</strong></p>
          <p className="text-sm text-[var(--color-text-secondary)] mb-2">On intègre sur D_{'{X/Y}'} = [y, 3] :</p>
          ...
          <p className="text-sm text-[var(--color-text-secondary)] mb-2">Formule : <M>{"E(Y|X=x) = \\int y \\cdot b^x(y) dy"}</M></p>
    ...
    },
    {
      ...
      solution: (
        <div className="space-y-4">
          {/* Méthode du professeur */}
          <div className="p-4 bg-[var(--color-warning-subtle)] rounded-lg border border-[var(--color-warning)]">
            <p className="font-semibold text-[var(--color-warning)] mb-3">📐 Méthode : Les 4 bornes</p>
            <div className="text-sm text-[var(--color-text-primary)]">
              <M>{"\\begin{cases} D_{0X} = [0, 2] \\\\ D_{0Y} = [0, 1] \\\\ D_{Y/X} = [0, \\min(x, 1)] & \\text{change à } x=1 \\\\ D_{X/Y} = [y, 2] \\end{cases}"}</M>
              <p className="mt-2 text-[var(--color-error)] font-medium">⚠️ Découper avec Chasles : ∫₀² = ∫₀¹ + ∫₁²</p>
            </div>
          </div>
          ...
          <div className="p-3 bg-[var(--color-success-subtle)] rounded border border-[var(--color-success)]">
            <M>{"a(x) = \\begin{cases} ax^3 + \\frac{2x^4}{3} & 0 \\leq x \\leq 1 \\\\ ax^2 + \\frac{2x}{3} & 1 < x \\leq 2 \\end{cases}"}</M>
          </div>
          
          <p><strong>3. E(X) :</strong></p>
          <p className="text-sm text-[var(--color-text-secondary)] mb-2">Découper aussi : <M>{"E(X) = \\int_0^1 x \\cdot a(x) dx + \\int_1^2 x \\cdot a(x) dx"}</M></p>
          <M>{"E(X) = \\boxed{1.463}"}</M>
        </div>
      ),
    },
    {
      ...
      solution: (
        <div className="space-y-4">
          ...
          <p><strong>Marginale b(y) :</strong></p>
          <p className="text-sm text-[var(--color-text-secondary)] mb-2">D_{'{X/Y}'} = [y, y+4] (largeur constante = 4)</p>
          <M>{"b(y) = \\frac{1}{4} \\int_y^{y+4} e^{-y} dx = \\frac{e^{-y}}{4} \\times 4 = e^{-y}"}</M>
          <p className="text-[var(--color-success)] font-semibold">→ Y ~ Exp(1) !</p>
          ...
          <p className="text-[var(--color-success)] font-semibold">→ X|Y=y ~ U([y, y+4]) !</p>
