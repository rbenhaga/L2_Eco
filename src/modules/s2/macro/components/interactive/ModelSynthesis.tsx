...
export function ModelSynthesis() {
  ...
  return (
    <div className="bg-white dark:bg-slate-900/80 rounded-2xl border border-gray-100 p-6 my-8 shadow-sm">
      {/* Tabs minimalistes */}
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
              activeTab === tab.id 
                ...
                : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-white hover:bg-slate-100/50 dark:bg-white/5'
            }`}
          >
            {tab.label}
          </button>
          ...
          {traps.map((trap, idx) => (
            ...
            >
              <button
                ...
                className="w-full px-4 py-3 flex items-center justify-between hover:bg-slate-100/50 dark:bg-white/5 transition-colors"
              >
                <div className="flex items-center gap-3">
                  <AlertTriangle className="w-4 h-4 text-amber-500" />
                  <span className="text-sm font-medium text-slate-900 dark:text-white">{trap.title}</span>
                </div>
                {expandedTrap === idx ? (
              ...
              {expandedTrap === idx && (
                <div className="px-4 pb-4 pt-2 border-t border-gray-50">
                  ...
                  <div className="p-2 bg-slate-100/50 dark:bg-white/5 rounded-lg inline-block">
      ...
      {activeTab === 'history' && (
        <div className="grid md:grid-cols-2 gap-3">
          {historicalExamples.map((ex, idx) => (
            <div 
              key={idx} 
              className="p-4 rounded-xl border border-gray-100 hover:border-slate-200 dark:border-white/10 transition-colors"
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <Clock className="w-4 h-4 text-slate-500 dark:text-slate-500" />
                  <span className="text-xs text-slate-600 dark:text-slate-400">{ex.year}</span>
                </div>
                <span 
              ...
              <p className="font-medium text-slate-900 dark:text-white mb-1">{ex.event}</p>
              <p className="text-sm text-slate-700 dark:text-slate-300">{ex.desc}</p>
            </div>
          ))}
        </div>
      )}
