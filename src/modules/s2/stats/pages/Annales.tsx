        ...
        <div className="space-y-4">
          {exams.map((exam) => (
            <button
              ...
              className="w-full p-6 bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-white/10 rounded-xl hover:border-gray-400 hover:shadow-md transition-all text-left group"
            >
    ...
    <main className="max-w-4xl mx-auto px-6 pt-24 pb-8">
      ...
      <div className="sticky top-16 bg-white dark:bg-slate-900/80 border-b border-slate-200 dark:border-white/10 -mx-6 px-6 py-4 mb-6 z-10">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-600 dark:text-slate-400">{answeredQuestions}/{totalQuestions} questions</p>
          </div>
          
          {showTimer && (
            <div className={`flex items-center gap-2 px-4 py-2 rounded-lg ${timeLeft < 600 ? 'bg-red-100 text-red-700' : 'bg-slate-100/80 dark:bg-white/5 text-slate-800 dark:text-slate-200'}`}>
              <Clock className="w-4 h-4" />
              <span className="font-mono font-bold">{formatTime(timeLeft)}</span>
            </div>
          )}
          
          <button onClick={() => setShowTimer(!showTimer)} className="text-sm text-slate-600 dark:text-slate-400 hover:text-slate-800 dark:text-slate-200">
      ...
      {exercise && question && (
        <div className="bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-white/10 rounded-xl p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-slate-600 dark:text-slate-400">{exercise.points} points</span>
          </div>

          {exercise.context && (
            <div className="p-4 bg-slate-100/50 dark:bg-white/5 rounded-lg mb-6 text-sm text-slate-800 dark:text-slate-200">
          ...
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-4">
              <span className="px-2 py-1 bg-slate-100/80 dark:bg-white/5 rounded text-sm font-medium text-slate-700 dark:text-slate-300">
                Q{currentQuestion + 1}/{exercise.questions.length}
              </span>
              <span className="text-sm text-slate-600 dark:text-slate-400">({question.points} pt{question.points > 1 ? 's' : ''})</span>
            </div>
            
            <p className="text-lg text-slate-900 dark:text-white mb-6">{renderWithMath(question.question)}</p>
                  ...
                  <button
                    ...
                    className={`w-full p-4 text-left rounded-lg border transition-all cursor-pointer ${
                      isSelected
                        ...
                        : 'border-slate-200 dark:border-white/10 hover:border-gray-400 text-slate-800 dark:text-slate-200'
                    }`}
                  >
      ...
      <div className="flex items-center justify-between">
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:text-white disabled:opacity-30 disabled:cursor-not-allowed"
        >
